---
name: ui-design
description: "Design and build production-grade frontend interfaces from scratch, covering the full arc: context gathering, reference collection, visual direction, implementation, and polish. Includes an explicit AI-slop test that names the tells to avoid. Use when: Creating a new page, component, or design system rather than auditing an existing one, Starting the UI for a new app on the homelab, The user asks for something that looks designed rather than generic, Establishing colour, type, and spacing decisions before writing code, A build looks like every other AI-generated interface and needs direction."
metadata:
  category: dev
---
...
Adapted from `ourostack/ouroboros-skills` (`skills/design`), renamed to avoid
colliding with the bundled `frontend-design` skill.
